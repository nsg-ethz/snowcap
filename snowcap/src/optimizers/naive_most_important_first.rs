...
pub struct NaiveMostImportantFirst<P: SoftPolicy + Clone> {
    net: Network,
...
impl<P: SoftPolicy + Clone> Optimizer<P> for NaiveMostImportantFirst<P> {
    fn new(
        mut net: Network,
        modifiers: Vec<ConfigModifier>,
    ...
    fn work(&mut self, _abort: Stopper) -> Result<(Vec<ConfigModifier>, f64), Error> {
        ...
        weighted_sequence.sort_by(|(_, a), (_, b)| b.cmp(a));
...
impl<P: SoftPolicy + Clone> NaiveMostImportantFirst<P> {
