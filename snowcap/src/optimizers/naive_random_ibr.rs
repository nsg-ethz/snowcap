...
use crate::strategies::{NaiveRandomIBRStrategy, Strategy};
...
/// orderings, until it succeeds. However, it always schedules insert, before update, before remove
/// commands.
pub struct NaiveRandomIBROptimizer<P> {
    net: Network,
...
impl<P> Optimizer<P> for NaiveRandomIBROptimizer<P>
where
    P: SoftPolicy + Clone,
{
    fn new(
    ...
    fn work(&mut self, abort: Stopper) -> Result<(Vec<ConfigModifier>, f64), Error> {
        let mut child = NaiveRandomIBRStrategy::new(
            self.net.clone(),
...
impl<P: SoftPolicy + Clone> NaiveRandomIBROptimizer<P> {
