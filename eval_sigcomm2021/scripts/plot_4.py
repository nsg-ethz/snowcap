...
if "-pre" in sys.argv:
    root_folder = "eval_sigcomm2021/precomputed_results/result_4"
else:
    root_folder = "eval_sigcomm2021/result_4"
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    xmin = 1,
    xmax = 21,
    xlabel = {Number of dependency groups},
    ylabel = {Time [s]},
    ...
    \addlegendimage{evaluation line legend={cBlue, thick}{fill=none, draw=none}}
    \addlegendentry{Snowcap$^-$}

    \addlegendimage{evaluation line legend={cGreen, thick}{fill=none, draw=none}}
    \addlegendentry{Snowcap}

    \addplot+[mark=none, thick, cRed] table[x=x, y=random, col sep=comma] {data.csv};
    \addplot+[mark=none, thick, cBlue] table[x=x, y=tree, col sep=comma] {data.csv};
    \addplot+[mark=none, thick, cGreen] table[x=x, y=snowcap, col sep=comma] {data.csv};

    \draw[fill=white, draw=none] (axis cs:2, 24) rectangle (axis cs:8, 42);
...
result_pdf_file = os.path.join(os.path.dirname(root_folder), "plot_4.pdf")
